 ...
 *
 * Modified on 1/27/12 by eden@erly.com, see `done` method.
 */
...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            // ----------------------------------------------
            // Modified on 1/27/12 by eden@erly.com
            //  - added e.rendered and rendered event trigger
            // ----------------------------------------------
                ...
                if (data.context) {
                    data.context.each(function (index) {
                        ...
                        $(this).fadeOut(function() {
                          if (!that) { return; }
                          e.rendered = that._renderDownload([file]).css('display', 'none').replaceAll(this).fadeIn(function() {
                            // Fix for IE7 and lower:
                            $(this).show();
                          });
                          that._trigger('rendered', e, data);
                        });
                    });
                } else {
                    e.rendered = that._renderDownload(data.result)
                        .css('display', 'none')
